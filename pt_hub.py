...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        def _open_coinbase_api_wizard() -> None:
            """
            Beginner-friendly wizard that stores Coinbase Advanced Trade API credentials.

            What we store:
              - cb_key.txt    = your Coinbase API Key (organizations/{org}/apiKeys/{key})
              - cb_secret.txt = your API Secret (EC PEM private key)
            """
            import webbrowser
            ...
            try:
                from coinbase.rest import RESTClient as _TestClient
            except Exception:
                messagebox.showerror(
                    "Missing dependency",
                    "The 'coinbase-advanced-py' package is required for Coinbase API setup.\n\n"
            ...
            # Instructions
            intro = (
                "This trader uses the Coinbase Advanced Trade API.\n\n"
                ...
                "  3) Select your portfolio and enable 'Trade' permission.\n"
                "  4) Complete 2FA verification.\n"
                "  5) Copy the API Key Name (looks like: organizations/{org_id}/apiKeys/{key_id}).\n"
                "  6) Copy or download the API Secret (a multi-line EC PEM private key starting with\n"
                "     -----BEGIN EC PRIVATE KEY-----).\n\n"
                "This wizard will save two files in the same folder as pt_hub.py:\n"
                "  - cb_key.txt    (your API Key Name)\n"
                "  - cb_secret.txt (your API Secret PEM key) -- keep this secret like a password\n"
            )
            ...
            def open_coinbase_page():
                webbrowser.open("https://portal.cdp.coinbase.com/access/api")

            ttk.Button(top_btns, text="Open Coinbase API Portal", command=open_coinbase_page).pack(side="left")
            ttk.Button(top_btns, text="Coinbase Advanced Trade Docs", command=lambda: webbrowser.open("https://docs.cdp.coinbase.com/advanced-trade/docs/welcome")).pack(side="left", padx=8)
            ttk.Button(top_btns, text="Open Folder With cb_key.txt / cb_secret.txt", command=lambda: _open_in_file_manager(self.project_dir)).pack(side="left", padx=8)

            # Step 1 — Paste API Key
            step1 = ttk.LabelFrame(container, text="Step 1 — Paste your Coinbase API Key Name")
            step1.grid(row=2, column=0, sticky="nsew", pady=(0, 10))
            step1.columnconfigure(0, weight=1)

            step1_help = (
                "From the Coinbase Developer Portal, copy the API Key Name.\n"
                "It looks like: organizations/{org_id}/apiKeys/{key_id}"
            )
            ttk.Label(step1, text=step1_help, justify="left").grid(row=0, column=0, sticky="w", padx=10, pady=(8, 0))
            ...
            # Step 2 — Paste API Secret (multi-line PEM)
            step2 = ttk.LabelFrame(container, text="Step 2 — Paste your Coinbase API Secret (PEM)")
            step2.grid(row=3, column=0, sticky="nsew", pady=(0, 10))
            step2.columnconfigure(0, weight=1)

            step2_help = (
                "Paste the full API Secret here. It is a multi-line EC PEM private key:\n"
                "-----BEGIN EC PRIVATE KEY-----\n"
                "...\n"
                "-----END EC PRIVATE KEY-----"
            )
            ...
            secret_box = tk.Text(step2, height=8, wrap="none")
            secret_box.grid(row=1, column=0, sticky="nsew", padx=10, pady=(6, 10))
            secret_box.configure(bg=DARK_PANEL, fg=DARK_FG, insertbackground=DARK_FG)
            if existing_api_secret:
                secret_box.insert("1.0", existing_api_secret)

            def _test_credentials() -> None:
                api_key = (api_key_var.get() or "").strip()
                api_secret = (secret_box.get("1.0", "end") or "").strip()

                if not api_key:
                    messagebox.showerror("Missing API key", "Paste the API Key Name into Step 1 first.")
                    return
                if not api_secret:
                    messagebox.showerror("Missing API secret", "Paste the API Secret PEM into Step 2 first.")
                    return

                try:
                    client = _TestClient(api_key=api_key, api_secret=api_secret)
                    ...
                    try:
                        if pricebooks:
                            ask = pricebooks[0]["asks"][0]["price"]
                    except Exception:
                        pass

                    messagebox.showinfo(
                        "Test successful",
                        "Your API Key + Secret worked!\n\n"
                        "Coinbase responded successfully.\n"
                        f"BTC-USD ask (example): {ask if ask is not None else 'received'}\n\n"
                        "Next: click Save."
                    )
                except Exception as e:
                    messagebox.showerror("Test failed", f"Couldn't authenticate with Coinbase.\n\nError:\n{e}")

            test_btns = ttk.Frame(step2)
            test_btns.grid(row=2, column=0, sticky="w", padx=10, pady=(0, 10))
            ttk.Button(test_btns, text="Test Credentials (safe, no trading)", command=_test_credentials).pack(side="left")
            ...
            ack = ttk.Checkbutton(
                step3,
                text="I understand cb_secret.txt is PRIVATE and I will not share it.",
                variable=ack_var,
            )
            ...
            def do_save():
                ...
                if not api_key:
                    messagebox.showerror("Missing API key", "Step 1: paste your Coinbase API Key Name first.")
                    return
                if not api_secret:
                    messagebox.showerror("Missing API secret", "Step 2: paste your API Secret PEM first.")
                    return
                if not bool(ack_var.get()):
                    messagebox.showwarning(
                        "Please confirm",
                        "For safety, please check the box confirming you understand cb_secret.txt is private."
                    )
                    return
                ...
                messagebox.showinfo(
                    ...
                    f"  API Secret → {_mask_path(secret_path)}\n\n"
                    "Next steps:\n"
                    "  1) Close this window\n"
        ...
        ttk.Button(api_row, text="Setup Wizard", command=_open_coinbase_api_wizard).grid(row=0, column=1, sticky="e", padx=(10, 0))
